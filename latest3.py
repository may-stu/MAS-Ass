...
TARGET_SUCCESS_RATE = 0.75      # stop when eval ≥ 75%
EVAL_INTERVAL       = 10000     # check every 10k episodes
AGENT_STEP_BUDGET   = 1_500_000 # global agent-step cap per assignment
...
def train(num_episodes=100000, collision_budget=3999, time_budget=600):
    ...
    for ep in range(1, num_episodes+1):
        # 0) Global budget check
        if total_steps >= AGENT_STEP_BUDGET:
            print(f"Reached global agent-step budget ({AGENT_STEP_BUDGET}) at episode {ep}, stopping training.")
            break
        ...
        # 2) Run the episode (up to 25 steps)
        ...
        # 6) In-training evaluation & early stop
        if ep % EVAL_INTERVAL == 0:
            current_rate = evaluate(shared_q)
            print(f"[Train Eval] after {ep} episodes → success rate {current_rate*100:.2f}%")
            if current_rate >= TARGET_SUCCESS_RATE:
                print(f"Target reached ({current_rate*100:.0f}%), stopping training at episode {ep}.")
                break

    # End of training loop

    # Report under-trained pairs
...
if __name__ == '__main__':
    # Load existing Q-table or train a new one
    if os.path.exists("q_table.pkl"):
        with open("q_table.pkl", "rb") as f:
            q_table = pickle.load(f)
        print("Loaded existing Q-table for evaluation.")
    else:
        print("No Q-table found; starting training...")
        q_table = train()
    # Evaluate the policy
    final_rate = evaluate(q_table)
    print(f"Overall success rate: {final_rate:.2f}%")
import os
import numpy as np
import random
import pickle
